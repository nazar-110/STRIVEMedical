...
/// <summary>
/// Mirrors firmware armStatusCode (USB.h). Values must stay in sync.
/// Reported via TELEM_STATUS.ArmStatus every 100 ms.
/// </summary>
public enum ArmOperatingMode : byte
{
    Bootup           = 0x00, // Hardware verification running
    Idle             = 0x01, // Waiting for host ping
    Connected        = 0x02, // Host connected; verifying subsystems
    ManualHomeAssist = 0x03, // Operator manually placing arm at home pose
    AssistActive     = 0x04, // Normal: admittance control active, arm follows user
    AutoReturnHome   = 0x05, // Autonomous return-to-home in progress
    RestoreAssist    = 0x06, // Re-enabling admittance after auto home
    PoweringOff      = 0x07, // Controlled shutdown
    Error            = 0x08, // Fault latched; operator must acknowledge
    Unknown          = 0xFF, // Not yet received from firmware
}
...
// TELEM_JOINT_DATA (Type: 0x84, Length: 52)
// 6 joints × 8 bytes (float angle + float vel) = 48
// + 1 byte TriggerPressed + 3 bytes C++ tail padding = 52 total
[StructLayout(LayoutKind.Sequential, Pack = 1)]
public struct Payload_TelemJointData
{
    public float J0_AngleRaw; public float J0_VelRaw;
    public float J1_AngleRaw; public float J1_VelRaw;
    public float J2_AngleRaw; public float J2_VelRaw;
    public float J3_AngleRaw; public float J3_VelRaw;
    public float J4_AngleRaw; public float J4_VelRaw;
    public float J5_AngleRaw; public float J5_VelRaw;
...
// CMD_SET_JOINT_TARGETS (Type: 0x04, Length: 48)
// 6 joints × (float vel + float torque) = 48 bytes
[StructLayout(LayoutKind.Sequential, Pack = 1)]
public struct Payload_SetJointTargets
{
    public float J0_Vel; public float J0_Torque;   // ROTATE
    public float J1_Vel; public float J1_Torque;   // REACH
    public float J2_Vel; public float J2_Torque;   // LIFT
    public float J3_Vel; public float J3_Torque;   // PITCH
    public float J4_Vel; public float J4_Torque;   // YAW
    public float J5_Vel; public float J5_Torque;   // ROLL
}
...
public class Packet
{
    ...
    public Packet(PacketType Type, byte[] Payload)
    {
        ...
        switch(Type)
        {
            ...
            case PacketType.CMD_SET_JOINT_TARGETS:
                Header.Length = 0x30; // 48 bytes: 6 joints × (float vel + float torque)
    ...
    public Packet(byte[] packet)
    {
        ...
        // Use numeric formatting on the underlying integer value to avoid Enum format exceptions
        Debug.Log($"Received Packet: Type=0x{((int)Header.Type):X2} Length={Header.Length} CRC Valid={IsValid}");
